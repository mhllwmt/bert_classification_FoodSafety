...
def load_dataset(filepath, max_len):
    ...
    r = csv.reader(f)
    for item in r:
        dataset_list.append(item[0].split('\t'))
        # print(item[0].split('\t'))
...
if __name__ == "__main__":
    ...
    # 生成数据集以及迭代器
    train_dataset = load_dataset('data/train_new.csv', max_len = MAX_LEN)  # 7337 * 3
    test_dataset = load_dataset('data/test_new.csv', max_len = MAX_LEN)  # 7356 * 3
    ...
    config.num_labels = 2
    model = BertForSequenceClassification.from_pretrained(file, config=config)
    model.cuda()
